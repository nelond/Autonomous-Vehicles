...
/*for building with S-function Builder...
Input Port: EN type Boolean
Output Port: a type double
Both are 1-D
*/
	...
	//Returns 2 bytes from slave
	//This specific example reads 0-15 of rotational bits
	int getData()
	{
	encoderVal = 0;
	START();	            //start bit
	...
	if ( SMACK() == 1); 	//we want an ack
		return 2; 			//error
	firstByte = READ();		//read one byte (bits 15-8)
	MasterACK(0);			//0 indicates ACK. We need lsbyte
	secondByte = READ();	//Read last Byte bits(7-0)
	STOP();					//stop bit
